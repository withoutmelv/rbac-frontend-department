...
const emit = defineEmits<{ logout: []; logoutPlay: [] }>();
...
const [LogoutPlayModal, logoutPlayModalApi] = useVbenModal({
  onConfirm() {
    emit('logoutPlay');
    logoutPlayModalApi.close();
  },
});
...
const isPlayer = computed<boolean>(() => {
  return accessStore.$state.isPlayer;
});
